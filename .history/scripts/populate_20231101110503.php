...
for ($i = 0; $i < $numRecords; $i++) {
    $user_id = getValidUserID($mysqli); // Assuming a random user_id.
    $product_id = getValidProductID($mysqli); // Assuming a random product_id.
    $quantity = rand(1, 10); // Assuming a random quantity.
    $order_date = $faker->dateTimeBetween('-1 year', 'now')->format('Y-m-d H:i:s');
    $status = $faker->randomElement(['pending', 'shipped']); // Randomly choose the order status.

    insertOrderData($user_id, $product_id, $quantity, $order_date, $status);
}
...
for ($i = 0; $i < $numRecords; $i++) {
    $symbol = $faker->symbol; // Use the correct formatter for currency symbol.
    $currencycode = $faker->currencycode;  // Use the correct formatter "currencyCode."

    insertCurrencyData($symbol,$currencycode);  // Update the insertCurrencyData function to accept the correct parameter.
}
...
for ($i = 0; $i < $numRecords; $i++) {
    $languageCode = $faker->languageCode;  // Use the correct formatter "languageCode."

    insertLanguageData($languageCode);  // Update the insertLanguageData function to accept the correct parameter.
}
