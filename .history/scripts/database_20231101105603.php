...
function insertCurrencyData($currency_name,$currencycode) {
    global $mysqli;

    $stmt = $mysqli->prepare("INSERT INTO currency (currency_name,currencycode) VALUES (?,?)");

    if (!$stmt) {
        die("Error: " . $mysqli->error);
    }

    $stmt->bind_param("ss", $currencycode);
...
function insertLanguageData($languageCode) {
    global $mysqli;

    $stmt = $mysqli->prepare("INSERT INTO language (languageCode) VALUES (?)");
